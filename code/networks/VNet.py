...
class Decoder(nn.Module):
    ...
    def forward(self, features):
        ...
        if self.has_dropout:
            x9 = self.dropout(x9)
        out = self.out_conv(x9)#加的
        out_tanh = self.tanh(out)#加的
        out_seg = self.out_conv(x9)#原来的
        # ruturn out_seg, out_tanh#加的
        # return out_seg, x9#原来的
        return out_seg, x9, out_tanh#换成X5试试
    
class Decoder_CML(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False, up_type=0):
        super(Decoder_CML, self).__init__()
        self.has_dropout = has_dropout
...
class VNet_CML(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False):
        super(VNet_CML, self).__init__()
        ...
        self.decoder1 = Decoder_CML(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, up_type=0)
        self.decoder2 = Decoder_CML(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, up_type=1)
    ...
    def forward(self, x):
        ...
        return out_seg1, out_seg2, embedding1, embedding2################0818加，我用的是这个，后面估计都要改
...
class VNet(nn.Module):
    ...
    def forward(self, input):
        ...
        out_seg, embedding, sdf = self.decoder(features)
        return out_seg, embedding, sdf # 4, 16, 112, 112, 80
    
if __name__ == '__main__':
    # compute FLOPS & PARAMETERS
    from thop import profile
    from thop import clever_format
    model = VNet(n_channels=1, n_classes=1, normalization='batchnorm', has_dropout=True)
