...
class Decoder_CML(nn.Module):
    def __init__(self, params):
        ...
        self.up1 = UpBlock_CML(self.ft_chns[4], self.ft_chns[3], self.ft_chns[3], dropout_p=0.0,
                           mode_upsampling=self.up_type)
        self.up2 = UpBlock_CML(self.ft_chns[3], self.ft_chns[2], self.ft_chns[2], dropout_p=0.0,
                           mode_upsampling=self.up_type)
        self.up3 = UpBlock_CML(self.ft_chns[2], self.ft_chns[1], self.ft_chns[1], dropout_p=0.0,
                           mode_upsampling=self.up_type)
        self.up4 = UpBlock_CML(self.ft_chns[1], self.ft_chns[0], self.ft_chns[0], dropout_p=0.0,
                           mode_upsampling=self.up_type)
...
class UNet_CML(nn.Module):
    def __init__(self, in_chns, class_num):
        super(UNet_CML, self).__init__()
        ...
        self.encoder = Encoder(params1)
        self.decoder1 = Decoder_CML(params1)
        self.decoder2 = Decoder_CML(params2)

    def forward(self, x):
        feature1 = self.encoder(x)
        feature2 = self.encoder(x)
        out_seg1, embedding1 = self.decoder1(feature1, with_feature=True)
        out_seg2, embedding2 = self.decoder1(feature2, with_feature=True)
