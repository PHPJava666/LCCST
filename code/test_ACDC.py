...
parser.add_argument('--model', type=str, default='unet_CML', help='model_name')
...
def Inference(FLAGS):
    ...
    # snapshot_path = "./model/CML/ACDC_{}_{}_labeled/{}".format(FLAGS.exp, FLAGS.labelnum, FLAGS.model)
    snapshot_path = "./model/CML/ACDC_{}_{}_labeled/{}".format(FLAGS.exp, FLAGS.labelnum, FLAGS.stage_name)
    test_save_path = "./model/CML/ACDC_{}_{}_labeled/{}_predictions/".format(FLAGS.exp, FLAGS.labelnum, FLAGS.model)
    if os.path.exists(test_save_path):
        shutil.rmtree(test_save_path)
    os.makedirs(test_save_path)
    net = net_factory(net_type=FLAGS.model, in_chns=1, class_num=FLAGS.num_classes)
    # save_model_path = os.path.join(snapshot_path, '{}_best_model.pth'.format(FLAGS.model))
    # save_model_path = os.path.join(snapshot_path, 'unet_CMLS2_best_model.pth')
    save_model_path = os.path.join(snapshot_path, 'iter_400_dice_0.6907S1.pth')
    # save_model_path = os.path.join(snapshot_path, 'unet_best_model_paper_ACDC_3_Labeled.pth')
    net.load_state_dict(torch.load(save_model_path))
    print("我在SS-Net-main文件夹0912假跑")
