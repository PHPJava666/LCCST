...
parser.add_argument('--model', type=str,  default='VNet_CML', help='model_name')
...
def test_calculate_metric():
    ...
    # save_model_path = os.path.join(snapshot_path, '{}_CMLS1_best_model.pth'.format(FLAGS.model))
    # save_model_path = os.path.join(snapshot_path, 'VNet_CMLS1_best_model.pth')
    # save_model_path = os.path.join(snapshot_path, 'iter_3600_dice_0.8125S1.pth')#UAMT
    # save_model_path = os.path.join(snapshot_path, 'iter_3400_dice_0.8119S1.pth')#4label
    save_model_path = os.path.join(snapshot_path, 'iter_3000_dice_0.8572S1.pth')#8label
    # save_model_path = os.path.join(snapshot_path, '{}_CMLS2_best_model.pth'.format(FLAGS.model))
    model.load_state_dict(torch.load(save_model_path))
    print("init weight from {}".format(save_model_path))
    print("我在SS-Net-main文件夹0911假跑")
    model.eval()
